...
fetch(`http://127.0.0.1:8000/api/postlist/?user=${id}`, {
  ...
  .then(data => {
    ...
    data.posts.forEach(post => {
      ...
      postElement.innerHTML = `
        <div class="main__post__single__header">
          ...
          <div class="main__post__single__header__edit__delete">
            <a class="main__post__single__header__edit" href="edit-post.html?id=${post.id}">Edit</a>
            <span>|</span>
            <a class="main__post__single__header__edit" data-post-id="${post.id}">Delete</a> 
          </div>
        </div>
        <hr>
    ...
    data.categories.forEach(category => {
      ...
      listItem.innerHTML = `<img class="main__cat__list__check" src="/assets/false_box.png" alt="" data-category-id="${category.id}">${category.name} (${category.user_post_count})`;
    ...
    function sendCategories() {
      ...
      const url = `http://127.0.0.1:8000/api/postlist/?categories=${categoryIds}&user=${id}`;  
      fetch(url, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
