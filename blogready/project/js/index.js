...
fetch('http://127.0.0.1:8000/api/postlist/', {
  ...
  .then(data => {
    ...
    data.categories.forEach(category => {
      ...
      listItem.innerHTML = `<img class="main__cat__list__check" src="assets/false_box.png" alt="" data-category-id="${category.id}">${category.name} (${category.post_count})`;
    ...
    function sendCategories() {
      ...
      const url = `http://127.0.0.1:8000/api/postlist/?categories=${categoryIds}`;  
      fetch(url, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
